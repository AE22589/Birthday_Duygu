...
const PUZZLE_COLS=3,PUZZLE_ROWS=4,PUZZLE_COUNT=PUZZLE_COLS*PUZZLE_ROWS,PUZZLE_EMPTY=PUZZLE_COUNT-1;
...
if(typeof document!=='undefined')(function(){
  ...
  function render(){boardEl.replaceChildren(...board.map((tile,index)=>{const button=document.createElement('button');button.type='button';button.className='pz-tile';button.dataset.index=String(index);button.setAttribute('role','gridcell');if(tile===PUZZLE_EMPTY){button.classList.add('is-empty');button.setAttribute('aria-label','Empty puzzle field');button.disabled=true}else{button.style.backgroundImage=`url("${PUZZLE_IMAGE}")`;button.style.backgroundPosition=backgroundPosition(tile);button.setAttribute('aria-label','Move puzzle tile');button.addEventListener('click',()=>move(index))}return button}))}
  ...
  function start(){board=puzzleShuffle();render();setView('game')}
  function back(){setView('intro');window.showQuestMap?.()}
  $('pzReadyButton')?.addEventListener('click',start);$('pzRetryButton')?.addEventListener('click',start);$('pzBackButton')?.addEventListener('click',back);$('pzGameBackButton')?.addEventListener('click',back);$('pzReturnButton')?.addEventListener('click',back);
