...
if(typeof document!=='undefined')(function(){
  ...
  let cards=[],open=[],matched=0,moves=0,locked=false,hideTimer=0,celebrationTimer=0;
  ...
  function complete(){locked=true;board.classList.add('is-complete-celebration');celebrationTimer=setTimeout(()=>{board.classList.remove('is-complete-celebration');saveCompletion();setView('result');},2500)}
  function flip(index){if(locked||open.length>=2)return;const card=cards[index];if(!card||card.open||card.matched)return;card.open=true;open.push(index);refreshCard(index);if(open.length<2)return;moves++;updateHud();const [a,b]=open;if(cards[a].id===cards[b].id){cards[a].matched=cards[b].matched=true;matched++;open=[];refreshCard(a);refreshCard(b);updateHud();if(matched===8)complete();return;}locked=true;hideTimer=setTimeout(()=>{cards[a].open=cards[b].open=false;open=[];locked=false;refreshCard(a);refreshCard(b);},650);}
  function reset(){clearTimeout(hideTimer);clearTimeout(celebrationTimer);board.classList.remove('is-complete-celebration');cards=shuffled().map(c=>({...c,open:false,matched:false}));open=[];matched=0;moves=0;locked=false;board.replaceChildren(...cards.map(renderCard));updateHud();}
