...
function renderFinalDoorReady(){
  finalDoorReadyLayer.replaceChildren();const count=[...effectiveCompleted()].filter(n=>Number.isInteger(n)&&n>=1&&n<=7).length;if(count!==7){finalDoorAnimatedOverlay.setAttribute('visibility','hidden');finalDoorAnimatedOverlay.style.opacity='0';return}finalDoorAnimatedOverlay.setAttribute('visibility','visible');finalDoorAnimatedOverlay.style.opacity='1';
...
returnVideo.addEventListener('ended',()=>{returnVideoEnd.hidden=false});
