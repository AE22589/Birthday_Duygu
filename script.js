...
function formatLockoutRemaining(ms){const totalMinutes=Math.ceil(ms/60000),h=Math.floor(totalMinutes/60),m=totalMinutes%60;return h>0?`${h}h ${m}m`:`${Math.max(1,m)}m`}
...
function updateLockoutUI(){if(isLockedOut()){error.textContent=`Too many attempts. Try again in ${formatLockoutRemaining(lockoutRemainingMs())}.`;codeInput.disabled=true;unlockButton.disabled=true}else{codeInput.disabled=false;unlockButton.disabled=false;if(lockoutTimer){clearInterval(lockoutTimer);lockoutTimer=null}}}
...
function openPreviewModal(context='entrance'){
  ...
  if(isLockedOut()){updateLockoutUI();if(!lockoutTimer)lockoutTimer=setInterval(updateLockoutUI,1000)}else{codeInput.disabled=false;unlockButton.disabled=false;requestAnimationFrame(()=>codeInput.focus())}
}
...
async function startReturnTimeTravel(){loadReturnAsset();const locked=isVideoLocked();returnVideo.controls=!locked;returnVideoPlay.hidden=locked;videoStamp.hidden=!locked;finalDoorModal.hidden=true;returnTravelScreen.hidden=false;returnVideoPanel.hidden=true;returnVideo.pause();returnVideo.currentTime=0;returnVideoEnd.hidden=true;returnVideoPlay.hidden=locked;returnTravelGif.hidden=true;returnTravelText.hidden=false;returnTravelText.className='return-travel-text';const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));for(const line of ['One last trip.','Hold on.',"Let's get you back home."]){returnTravelText.textContent=line;returnTravelText.classList.remove('is-visible');void returnTravelText.offsetWidth;returnTravelText.classList.add('is-visible');await wait(2500);returnTravelText.classList.remove('is-visible');await wait(250)}returnTravelText.hidden=true;returnTravelGif.hidden=false;await wait(4800);returnTravelGif.hidden=true;returnVideoPanel.hidden=false;if(locked){returnTravelScreen.hidden=true;oneLastChoice.hidden=false}}
...
unlockButton.addEventListener('click',()=>{
  ...
  if(codeInput.value.trim()!==ADMIN_CODE){
    ...
    if(isLockedOut()){updateLockoutUI();if(!lockoutTimer)lockoutTimer=setInterval(updateLockoutUI,1000)}
    return;
  }
  clearLockout();
